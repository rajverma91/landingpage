import image1 from "../../public/assets/img/portfolio/portfolio-6.png";
import image2 from "../../public/assets/img/portfolio/portfolio-5.png";
import image3 from "../../public/assets/img/portfolio/portfolio-4.png";
import image4 from "../../public/assets/img/portfolio/portfolio-3.png";
import image5 from "../../public/assets/img/portfolio/portfolio-2.jpg";
import image6 from "../../public/assets/img/portfolio/portfolio-1.jpg";

const portfolioData = [
    {        
        id: 'immersive-3D-twins',
        title: 'Immersive 3D Twins',
        subtitle: 'Experience the Future of Real Estate',
        image: image1,    
        details: {
            listdata1: `
           <h3 class="main"> What is 3D Twin Lite?</h3>
3D Twin Lite is an advanced digital twin technology that creates an interactive, 3D representation of properties and developments. It allows potential buyers, investors, and stakeholders to explore properties in a highly immersive environment, enhancing engagement and decision-making.

<h3>How It Works</h3>
<ul><li><strong>Realistic 3D Modeling:</strong> Creates a virtual replica of buildings, interiors, and surrounding environments.</li>
<li><strong>Interactive Walkthroughs:</strong> Users can navigate through spaces, view different units, and get a feel for the property before visiting.</li>
<li><strong>Data-Driven Insights:</strong> Integrates with property databases to provide real-time availability, pricing, and specifications.</li>
<li><strong>VR & AR Compatibility:</strong> Enhances the experience with Virtual and Augmented Reality support.</li></ul>

<h3>Why Your Business Needs 3D Twin Lite</h3>
<ul><li><strong>Enhanced Property Showcase:</strong> Give clients an engaging, high-quality view of your properties.</li>
<li><strong>Remote Access:</strong> Buyers can explore properties from anywhere in the world, increasing your reach.</li>
<li><strong>Accelerated Sales Cycle:</strong> Reduce site visits and inquiries by providing comprehensive digital previews.</li>
<li><strong>Improved Decision Making:</strong> Clients get a complete picture of the space, reducing uncertainties and increasing conversions.</li></ul>

<h3>What Borning Studios Offers</h3>
<ul><li>Custom 3D Property Visualization</li>
<li>Aerial 360° Views</li>
<li>Interactive UI for Easy Navigation</li>
<li>Integration with Your CRM & Website</li>
<li>VR/AR-Enabled Experiences</li>
<li>Data Insights & Real-Time Availability Updates</li></ul>

<h3>Key Benefits</h3>
<ul><li> Higher Engagement – Users spend more time exploring interactive 3D models.</li>
<li> Increased Conversions – Buyers gain confidence with immersive digital tours.</li>
<li> Cost-Effective – Reduce physical visits and print material costs.</li>
<li> Competitive Edge – Stay ahead of the competition with cutting-edge visualization.</li></ul>
            `
         }   
    },
    {        
        id: 'step-inside-virtually',
        title: 'Step Inside, Virtually',
        subtitle: 'Real Estate Tours',
        image: image2,
        details: {
            listdata1: `
          <h3 class="main">   Step Inside, Anytime, Anywhere </h3> 
Imagine walking through your dream home before even setting foot inside. With Virtual Tours, your clients get a real feel of the property, exploring every corner as if they were actually there. No guesswork, no surprises—just an immersive, interactive experience that makes decision-making easier and more exciting.
 <h3> Why This Changes the Game for Real Estate </h3> 
   Buying or renting a property isn’t just about numbers—it’s about feeling at home. A 360° Virtual Tour does more than showcase a space; it lets buyers experience it. Whether they’re in another city or just short on time, they can tour the property whenever and wherever they want.
 <h3> Why Your Listings Need Virtual Tours </h3> 
<ul><li> 24/7 Open House – Your properties are always available for viewing, no scheduling required.</li>
<li> Serious Buyers, Less Legwork – Buyers can explore online, filtering their choices before in-person visits.</li>
<li> Better Engagement, Faster Sales – A virtual experience builds excitement, helping buyers take action sooner.</li>
<li>Reach More Buyers – International and remote clients can tour the property without the travel hassle.</li></ul>
<h3>What Borning Studios Brings to the Table</h3>
We don’t just create tours; we craft experiences. With high-resolution 360° imagery, smooth navigation, and interactive elements, we make sure your properties don’t just get seen—they get remembered.
 <ul><li>Full 360° Virtual Walkthroughs – Let clients navigate freely, just like they would in person.</li>
<li> Stunning HD Panoramas – Every detail matters, and we make sure it’s crystal clear.</li>
<li> Interactive Hotspots – Add property details, highlights, or even video messages.</li>
<li> Custom Branding & UI – Your tours, your style. We tailor it to your brand.</li>
<li> VR-Ready Experiences – Take it a step further with full VR compatibility for an even more immersive feel.</li> </ul>
<h3>Let’s Make Your Listings Stand Out</h3>
A picture is worth a thousand words, but a virtual tour can close a deal. Whether you're a real estate agent, developer, or property manager, this is your chance to stay ahead of the curve and make your listings truly stand out.
<h3>👉 Let’s create something unforgettable – Get in touch today!</h3>
            `
         }  
    },
    {        
        id: 'step-into-your-vision',
        title: 'Step Into Your Vision',
        subtitle: 'Explore Villas in Stunning 360° Detail',
        image: image3,
        details: {
            listdata1: `
           <h3 class="main"> Portfolio Page Content - 3D 360° Outer & Inner Elevation View of Villa</h3>
<h3>Experience Luxury Like Never Before</h3>
Your dream villa deserves more than static images—it deserves to be felt. With our 3D 360° elevation views, we bring your property to life, allowing clients to walk through, explore, and experience every architectural detail inside and out.
<h3>What Makes This So Powerful?</h3>
Most buyers struggle to visualize a home from traditional floor plans and photos. With our immersive 3D Outer & Inner Elevation Views, they see the property in its full glory, making decisions faster and with confidence.
<h3>How This Helps You Sell Faster</h3>
<ul><li> Full 360° Exterior & Interior Walkthrough – Showcase the villa’s design, space, and aesthetics with precision.</li>
<li>Engage Buyers Like Never Before – Let them experience the ambiance and details before stepping in.</li>
<li> Boost Buyer Confidence – With a full 3D visualization, clients feel assured about their choice.</li>
<li> Perfect for Architects & Developers – Present your project in an immersive, high-quality format.</li></ul>
<h3>What Borning Studios Offers</h3>
At Borning Studios, we don’t just render villas—we create immersive experiences. Our 3D 360° solutions ensure that your property isn’t just seen but felt.
<ul><li> <strong>Crystal-Clear 3D Renders –</strong> High-resolution, lifelike detailing.</li>
<li><strong> Interactive Walkthroughs – </strong>Clients can explore with full control.</li>
<li> <strong>Day & Night Views – </strong>Showcase lighting, ambiance, and seasonal aesthetics.</li>
<li> <strong>Custom Branding & UI – </strong>Make it fully tailored to your brand and project.</li>
<li> <strong>VR-Ready – </strong>For the ultimate immersive property tour experience.</li></ul>
<h3>See It, Experience It, Own It</h3>
Turn interest into investment by giving buyers and investors a real feel of your villa before they even visit. Let’s build an experience that turns vision into reality.
<h3>👉 Ready to showcase your villa like never before? Contact us today!</h3>
            `
         }  
    },
    {        
        id: 'step-inside-virtually',
        title: 'Step Inside, Virtually',
        subtitle: 'Explore Every Corner of Your Future Home',
        image: image4,  
        details: {
            listdata1: `
        <h3 class="main">   Portfolio Page Content - Virtual Reality Tour of Flat</h3>
<h3>Experience Your Home Before It’s Built</h3>
Why wait for site visits when you can explore your future home from anywhere? With our VR Flat Tours, step inside, walk around, and feel the space—just like a real visit.
<h3>Why Virtual Reality Tours?</h3>
<ul><li> Seamless, Interactive Experience – Move from room to room with a simple click.</li>
<li> High-Definition Views – Every texture, finish, and design detail is captured in crisp clarity.</li>
<li> Time-Saving & Efficient – No need for multiple site visits; make confident decisions remotely.</li>
<li> Perfect for Homebuyers & Investors – Compare multiple properties effortlessly.</li></ul>
<h3>How It Works</h3>
<ul><li> 360° Walkthroughs – Navigate through the entire flat in an immersive experience.</li>
<li> Detailed Room Overviews – See dimensions, layouts, and decor in real-time.</li>
<li> Custom Branding – Developers & realtors can personalize tours with logos and information.</li>
<li> Mobile & VR Compatible – Access the tour on any device or dive in fully with a VR headset.</li> </ul>
<h3>Why Choose Borning Studios?</h3>
We bring properties to life before they are even built. Whether you're a real estate developer, broker, or homebuyer, our VR property tours provide an unmatched level of engagement and trust.
<h3>👉 Take a Virtual Tour & Experience the Future of Home Buying!</h3>

            `
         }        
    },
   
];

export default portfolioData;